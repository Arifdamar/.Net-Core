...
namespace JWTAuthentication_Front.ApiServices.Concrete
{
    public class ProductApiManager : IProductApiService
    {
        ...
        public async Task AddAsync(ProductAdd productAdd)
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                ...
                var responseMessage = await httpClient.PostAsync("http://localhost:10188/api/products", stringContent);
            }
        }
        ...
        public async Task DeleteAsync(int id)
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                using var httpClient = new HttpClient();
                httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                await httpClient.DeleteAsync($"http://localhost:10188/api/products/{id}");
            }
        }
        ...
        public async Task<List<ProductList>> GetAllAsync()
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                using var httpClient = new HttpClient();
                httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                var responseMessage = await httpClient.GetAsync("http://localhost:10188/api/products");
        ...
        public async Task<ProductList> GetByIdAsync(int id)
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                using var httpClient = new HttpClient();
                httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                var responseMessage = await httpClient.GetAsync($"http://localhost:10188/api/products/{id}");
        ...
        public async Task UpdateAsync(ProductList productList)
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                ...
                var stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
                await httpClient.PutAsync("http://localhost:10188/api/products", stringContent);
            }
        }
    }
}